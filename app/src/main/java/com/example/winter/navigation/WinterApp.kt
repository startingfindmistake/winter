...
import com.example.winter.ui.screens.notes.addEditScreen.AddEditRoute
import com.example.winter.ui.screens.notes.homeScreen.NoteScreen
import com.example.winter.ui.screens.notes.trash.trashNoteDest
import com.example.winter.ui.screens.settingsScreen.SettingsScreen

@Composable
fun WinterApp(navController: NavHostController = rememberNavController()) {
    val bottomNavItem = getBottomNavItems()
    val screensWithHiddenNavBar = listOf(
        "${WinterScreens.AddEditNotes.name}/{noteId}",
        WinterScreens.TrashNoteScreen.name
    )
    ...
    Scaffold(
        ...
        NavHost(
            ...
            startDestination = WinterScreens.Notes.name,
            ...
            composable(
                route = WinterScreens.Notes.name
            ) {
                NoteScreen(
                    onFabClicked = { navController.navigate(WinterScreens.AddEditNotes.name + "/-1") },
                    navigateToUpdateNoteScreen = { noteId ->
                        navController.navigate("${WinterScreens.AddEditNotes.name}/$noteId")
                    }
                ) { shouldHide ->
            ...
            composable(
                route = "${WinterScreens.AddEditNotes.name}/{noteId}",
            ...
            composable(
                route = WinterScreens.Settings.name
            ) {
